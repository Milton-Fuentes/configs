...
import com.atser.tools.atserejb.service.glob.GlobDisciplinesService;
 ...
 * @author droldan
 */
@Path("/glob/disciplines")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobDisciplinesRS implements IBaseWSException {

    @EJB
    GlobDisciplinesService disciplinesService;
    ...
    public Object filter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return disciplinesService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object list(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return disciplinesService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return disciplinesService.findById(request.getBody().getParams().get("id").toString());
            } else {
