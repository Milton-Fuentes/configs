...
import com.atser.tools.atserejb.service.glob.GlobServicesLabCatService;
...
/**
 *
 * @author droldan Query Parameters f: filter o: order c: count
 */
@Path("/glob/servicelabcat")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobServicesLabCatRS implements IBaseWSException {

    @EJB
    GlobServicesLabCatService labCatService;
    ...
    public Object labCatGetServiceList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return labCatService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object labCatServiceList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return labCatService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return labCatService.findById(request.getBody().getParams().get("id").toString());
            } else {
