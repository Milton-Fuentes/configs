...
import com.atser.tools.atserejb.service.glob.GlobClientMaterialUseService;
 ...
 * @author droldan
 */
@Path("/glob/clientmaterialuse")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobClientMaterialUseRS implements IBaseWSException {

    @EJB
    private GlobClientMaterialUseService globClientMaterialUseService;
    ...
    public Object filterByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globClientMaterialUseService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globClientMaterialUseService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return globClientMaterialUseService.findById(request.getBody().getParams().get("id").toString());
            } else {
