...
import com.atser.tools.atserejb.service.cm.CmEmEmailReceiverService;
 ...
 * @author dperez
 */
@Path("/cm/em/emailreceiver")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class CmEmEmailReceiverRS implements IBaseWSException {

    @EJB
    CmEmEmailReceiverService emailReceiverService;

    @POST
    @Path("/list")
    public Object fileList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return emailReceiverService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return emailReceiverService.findById(request.getBody().getParams().get("id").toString());
            } else {
