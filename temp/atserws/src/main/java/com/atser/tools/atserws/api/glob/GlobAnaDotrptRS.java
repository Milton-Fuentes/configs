...
import com.atser.tools.atserejb.service.glob.GlobAnaDotrptService;
...
@ManageITSegmentRest
@Slf4j
public class GlobAnaDotrptRS implements IBaseWSException {

    @EJB
    GlobAnaDotrptService globAnaDotrptService;
    ...
    public Object clientFilter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globAnaDotrptService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object clientList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globAnaDotrptService.list(request.getBody());
        }
