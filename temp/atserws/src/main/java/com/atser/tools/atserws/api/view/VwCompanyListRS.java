...
import com.atser.tools.atserejb.service.view.VwCompanyListService;
 ...
 * @author dperez
 */
@Path("/view/vwcompanylist")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class VwCompanyListRS implements IBaseWSException {

    @EJB
    VwCompanyListService vwCompanyListService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return vwCompanyListService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return vwCompanyListService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return vwCompanyListService.list(ApiCommon.buildFilter(uriInfo));
    }

    @GET
    @Path("/listnames")
    public Object listDistinctByClient(@QueryParam("clientId") String clientId) throws JsonParseException, JsonMappingException, IOException {
        if (StringSupport.isNotNullAndNotEmpty(clientId) && NumberSupport.isNumeric(clientId)) {
            return vwCompanyListService.listNamesByClient(new BigInteger(clientId));
        } else {
            throwBaseException(Response.Status.BAD_REQUEST, "Missing [clientId] param.");
        }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return vwCompanyListService.list(request.getBody());
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return vwCompanyListService.findById(request.getBody().getParams().get("id").toString());
            } else {
