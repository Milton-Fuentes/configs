...
import com.atser.tools.atserws.api.glob.*;
import com.atser.tools.atserejb.service.glob.GlobProjectService;
import com.atser.tools.atserejb.service.qa.QaWizardItemService;
...
@ManageITSegmentRest
@Slf4j
public class QaWizardItemRS implements IBaseWSException {

    @EJB
    QaWizardItemService wizardItemService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return wizardItemService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object projectFilter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return wizardItemService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object projectList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return wizardItemService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return wizardItemService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @GET
    @Path("/next")
    public Object findNext(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return wizardItemService.findNext(ApiCommon.buildFilter(uriInfo));
    }
