...
import com.atser.tools.atserejb.service.glob.GlobProjectSupplierService;
 ...
 * @author droldan
 */
@Path("/glob/projectsupplier")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobProjectSupplierRS implements IBaseWSException {

    @EJB
    GlobProjectSupplierService globProjectSupplierService;
    ...
    public Object filter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globProjectSupplierService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object list(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globProjectSupplierService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return globProjectSupplierService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/getprojsuppid")
    public Object getProjectSupplierId(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams() == null || !request.getBody().getParams().isContainsAllKeysAndNotNull(new String[]{"projectId", "supId"})) {
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
            }
            return globProjectSupplierService.getProjectSupplierId(request.getBody());
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
    ...
    public Response generateSupplierReport(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws IOException, Exception {
        ...
        if (StringSupport.isNotNullAndNotEmpty(paramReportId)) {
            ...
            String filePath = globProjectSupplierService.generateReport(paramReportId, tplbasePath, docpathsegment, paramDocType, request.getHeader().getClientid(), request.getBody());
            if (filePath != null) {
