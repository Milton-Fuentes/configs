...
import com.atser.tools.atserejb.service.qa.QaaspmMixdesignJmfsvMasterService;
 ...
 * @author droldan
 */
@Path("/qa/aspmmixdesignjmfsvmaster")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class QaaspmMixdesignJmfsvMasterRS implements IBaseWSException {
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignJmfsvMasterService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignJmfsvMasterService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignJmfsvMasterService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return qaaspmMixdesignJmfsvMasterService.list(request.getBody());
        }
    ...
    @POST
    @Path("/save/batch")
    public Object crudBatch(ManageITRequest<List<VwQaaspmMixJmfsvMaster>> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null || request.getBody().getData().isEmpty()) {
                throwBaseException(Response.Status.BAD_REQUEST, "No data.");
            }
            qaaspmMixdesignJmfsvMasterService.updateBatch(request.getHeader(), request.getBody().getData());
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<QaaspmMixdesignJmfsvMaster> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return qaaspmMixdesignJmfsvMasterService.findById(request.getHeader(), request.getBody().getParams().get("id").toString());
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        qaaspmMixdesignJmfsvMasterService.delete(request.getBody().getParams().get("id").toString());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "insert": {
                        ...
                        qaaspmMixdesignJmfsvMasterService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        qaaspmMixdesignJmfsvMasterService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignJmfsvMasterService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignJmfsvMasterService.getRevisions(id);
    }
}
