 ...
 * @author dperez
 */
@Path("/bpm/correspondence/metadata")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class BpmCorrespondenceMetadataRS implements IBaseWSException {

    @EJB
    protected InitializeEnv env;

    @EJB
    BpmCorrespondenceMetaDataService bpmCorrespondenceMetaDataService;
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return bpmCorrespondenceMetaDataService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return bpmCorrespondenceMetaDataService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return bpmCorrespondenceMetaDataService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/findby/correspondenceid")
    public Object findByRfiId(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return bpmCorrespondenceMetaDataService.findByCorrespondenceId(new BigDecimal(request.getBody().getParams().get("id").toString()));
            } else {
                throwBaseException(Response.Status.BAD_REQUEST, "Missing [id] param.");
            }
        }
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<Bpmcorrespondencemetadata> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getAction())) {
                ...
                switch (action) {
                    ...
                    case "remove": {
                        ...
                        bpmCorrespondenceMetaDataService.delete(request.getBody());
                    ...
                    case "insert": {
                        ...
                        bpmCorrespondenceMetaDataService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        bpmCorrespondenceMetaDataService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
