...
import com.atser.tools.atserejb.service.glob.GlobQacomplianceLogService;
 ...
 * @author droldan
 */
@Path("/glob/qacompliancelog")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobQacomplianceLogRS implements IBaseWSException {

    @EJB
    private GlobQacomplianceLogService globQacomplianceLogService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globQacomplianceLogService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globQacomplianceLogService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globQacomplianceLogService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globQacomplianceLogService.list(request.getBody());
        }
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<GlobQacomplianceLog> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return globQacomplianceLogService.findById(request.getHeader(), request.getBody().getParams().get("id").toString());
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        globQacomplianceLogService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        globQacomplianceLogService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        globQacomplianceLogService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object changeStatus(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            ...
            globQacomplianceLogService.changeStatus(request.getBody());
        }
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return globQacomplianceLogService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return globQacomplianceLogService.getRevisions(id);
    }

    @POST
    @Path("/search")
    public Object search(ManageITRequest<GlobQacomplianceLogSearchDefParam> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null) {
                log.error("Wrong Request.");
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
            }
            return globQacomplianceLogService.getFrontEndSearch(request.getHeader(), request.getBody().getData());
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
    ...
    public Response generatePostReport(ManageITRequest<GlobQacomplianceLogSearchDefParam> request) throws IOException, Exception {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null) {
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
            }
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getData().reportId)) {
                ...
                String filePath = globQacomplianceLogService.generateReport(tplbasePath, docpathsegment, request.getHeader(), request.getBody().getData());
                if (filePath != null) {
