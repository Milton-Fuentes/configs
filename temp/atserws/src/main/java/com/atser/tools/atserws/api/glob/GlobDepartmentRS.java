...
import com.atser.tools.atserejb.service.glob.GlobDepartmentService;
 ...
 * @author droldan
 */
@Path("/glob/department")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobDepartmentRS implements IBaseWSException {

    @EJB
    GlobDepartmentService globDepartmentService;
    ...
    public Object clientFilter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globDepartmentService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object clientList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globDepartmentService.list(request.getBody());
        }
