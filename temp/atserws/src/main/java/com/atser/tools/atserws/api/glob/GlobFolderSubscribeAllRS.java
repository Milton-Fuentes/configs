...
import com.atser.tools.atserejb.service.glob.GlobFolderSubscribeAllService;
 ...
 * @author droldan
 */
@Path("/glob/foldersubscribeall")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobFolderSubscribeAllRS implements IBaseWSException {

    @EJB
    protected InitializeEnv env;

    @EJB
    private GlobFolderSubscribeAllService globFolderSubscribeAllService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globFolderSubscribeAllService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globFolderSubscribeAllService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globFolderSubscribeAllService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globFolderSubscribeAllService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return globFolderSubscribeAllService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<GlobFolderSubscribeAll> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getAction())) {
                ...
                switch (action) {
                    ...
                    case "remove": {
                        ...
                        globFolderSubscribeAllService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        return globFolderSubscribeAllService.save(request.getHeader(), request.getBody().getData());
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
                        }
                        globFolderSubscribeAllService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object exists(@QueryParam("fId") String folderId, @QueryParam("sId") String subscriberId) throws JsonParseException, JsonMappingException, IOException {
          if (StringSupport.isNotNullAndNotEmpty(folderId) && NumberSupport.isNumeric(folderId) && StringSupport.isNotNullAndNotEmpty(subscriberId) && NumberSupport.isNumeric(subscriberId)) {
            return globFolderSubscribeAllService.extists(new BigInteger(folderId), new BigInteger(subscriberId));
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
}
