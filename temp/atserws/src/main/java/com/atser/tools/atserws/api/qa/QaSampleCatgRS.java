...
@ManageITSegmentRest
@Slf4j
public class QaSampleCatgRS implements IBaseWSException {

    @EJB
    QaSampleCatgService qaSampleCatgService;
    ...
    public Object projectFilter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaSampleCatgService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object projectList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return qaSampleCatgService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return qaSampleCatgService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/list/byclient")
    public Object sampleCatgListByClient(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return qaSampleCatgService.getsampleCatgListByClient(request.getHeader(), request.getBody());
        }
