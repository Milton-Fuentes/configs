...
import com.atser.tools.atserejb.service.bpm.BpmProcessTrackingService;
 ...
 * @author droldan
 */
@Path("/bpm/processtracking")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class BpmProcessTrackingRS implements IBaseWSException {

    @EJB
    private BpmProcessTrackingService bpmProcessTrackingService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return bpmProcessTrackingService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return bpmProcessTrackingService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return bpmProcessTrackingService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return bpmProcessTrackingService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return bpmProcessTrackingService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<BpmProcessTracking> request) throws JsonParseException, JsonMappingException, IOException, ProcessExecutionException, ContractViolationException, BonitaException {
        if (request != null && request.validate()) {
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getAction())) {
                ...
                switch (action) {
                    ...
                    case "remove": {
                        ...
                        bpmProcessTrackingService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        return bpmProcessTrackingService.saveAndGet(request.getHeader(), request.getBody().getData());
                    }
                    case "register": {
                        if (request.getBody().getParams() == null || !request.getBody().getParams().isContainsAllKeysAndNotNull(new String[]{"projectId", "functionality", "process", "refId"})) {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong request.");
                        }
                        return bpmProcessTrackingService.saveAndGet(request.getHeader(), request.getBody());
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
                        }
                        bpmProcessTrackingService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
