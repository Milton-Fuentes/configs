...
import com.atser.tools.atserejb.service.qa.test.QasteelMagneticPartoutvService;
 ...
 * @author mfuentes
 */
@Path("/qa/steelmagneticpartoutv")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class QasteelMagneticoutvPartRS implements IBaseWSException {

    @EJB
    private QasteelMagneticPartoutvService service;
    @EJB
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<QasteelMagneticPartoutv> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return service.findById(NumberSupport.getBigDecimal(request.getBody().getParams().get("id")));
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        service.delete(request.getBody().getParams().get("id").toString());
