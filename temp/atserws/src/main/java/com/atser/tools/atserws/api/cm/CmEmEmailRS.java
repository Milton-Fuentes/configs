...
import com.atser.tools.atserejb.service.cm.CmEmEmailService;
 ...
 * @author dperez
 */
@Path("/cm/em/emailservice")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class CmEmEmailRS implements IBaseWSException {

    @EJB
    CmEmEmailService emailService;

    @POST
    @Path("/list")
    public Object fileList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return emailService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return emailService.findById(request.getBody().getParams().get("id").toString());
            } else {
