...
import com.atser.tools.atserejb.service.qa.QaInspectionNotuitemService;
 ...
 * @author droldan
 */
@Path("/qa/inspectionnotuitem")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class QaInspectionNotuitemRS implements IBaseWSException {
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaInspectionNotuitemService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaInspectionNotuitemService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filter(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaInspectionNotuitemService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object list(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return qaInspectionNotuitemService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return qaInspectionNotuitemService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<QaInspectionNotuitem> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return qaInspectionNotuitemService.findById(StringSupport.getString(request.getBody().getParams().get("id")));
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        qaInspectionNotuitemService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        qaInspectionNotuitemService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        qaInspectionNotuitemService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object changeStatus(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            ...
            qaInspectionNotuitemService.changeStatus(request.getBody());
        }
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return qaInspectionNotuitemService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return qaInspectionNotuitemService.getRevisions(id);
    }
}
