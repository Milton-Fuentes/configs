...
import com.atser.tools.atserejb.service.qa.test.QasteelMagneticPartService;
 ...
 * @author mfuentes
 */
@Path("/qa/steelmagneticpart")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class QasteelMagneticPartRS implements IBaseWSException {

    @EJB
    private QasteelMagneticPartService service;
    @EJB
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<QaGenericTestTO<QasteelMagneticPart>> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return service.findByTestIdTO(NumberSupport.getBigInteger(request.getBody().getParams().get("id")));
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "insert": {
                        ...
                        qaSampleTestAddInfoService.save(request.getHeader(), request.getBody().getData().getQaSampleTestAddInfo());
                        return service.save(request.getHeader(), request.getBody().getData().getTest());
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
                        }
                        service.updateTO(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
