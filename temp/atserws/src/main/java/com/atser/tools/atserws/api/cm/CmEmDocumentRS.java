...
import com.atser.tools.atserejb.service.cm.CmEmDocumentService;
 ...
 * @author dperez
 */
@Path("/cm/em/documentservice")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class CmEmDocumentRS implements IBaseWSException {

    @EJB
    CmEmDocumentService emDocumentService;

    @POST
    @Path("/list")
    public Object fileList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return emDocumentService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return emDocumentService.findById(request.getBody().getParams().get("id").toString());
            } else {
