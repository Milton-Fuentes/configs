...
import com.atser.tools.atserejb.service.glob.GlobFileTkShareService;
 ...
 * @author dperez
 */
@Path("/glob/filetkshare")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobFileTkShareRS implements IBaseWSException {

    @EJB
    GlobFileTkShareService fileTkService;

    @POST
    @Path("/list")
    public Object fileList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return fileTkService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return fileTkService.findById(request.getBody().getParams().get("id").toString());
            } else {
