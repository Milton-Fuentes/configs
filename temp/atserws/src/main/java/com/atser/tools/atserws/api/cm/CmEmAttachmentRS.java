...
import com.atser.tools.atserejb.service.cm.CmEmAttachmentService;
 ...
 * @author dperez
 */
@Path("/cm/em/attachment")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class CmEmAttachmentRS implements IBaseWSException {

    @EJB
    CmEmAttachmentService attachmentService;

    @POST
    @Path("/list")
    public Object fileList(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return attachmentService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return attachmentService.findById(request.getBody().getParams().get("id").toString());
            } else {
