...
import com.atser.tools.atserejb.service.glob.GlobCostmgmtLogService;
 ...
 * @author droldan
 */
@Path("/glob/costmgmtlog")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobCostmgmtLogRS implements IBaseWSException {

    @EJB
    private GlobCostmgmtLogService globCostmgmtLogService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globCostmgmtLogService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globCostmgmtLogService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globCostmgmtLogService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globCostmgmtLogService.list(request.getBody());
        }
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<GlobCostmgmtLog> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return globCostmgmtLogService.findById(request.getHeader(), request.getBody().getParams().get("id").toString());
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        globCostmgmtLogService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        globCostmgmtLogService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        globCostmgmtLogService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object changeStatus(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            ...
            globCostmgmtLogService.changeStatus(request.getBody());
        }
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return globCostmgmtLogService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return globCostmgmtLogService.getRevisions(id);
    }

    @POST
    @Path("/search")
    public Object search(ManageITRequest<GlobCostmgmtLogSearchDefParam> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null) {
                log.error("Wrong Request.");
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
            }
            return globCostmgmtLogService.getFrontEndSearch(request.getHeader(), request.getBody().getData());
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
    ...
    public Response generatePostReport(ManageITRequest<GlobCostmgmtLogSearchDefParam> request) throws IOException, Exception {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null) {
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
            }
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getData().reportId)) {
                ...
                String filePath = globCostmgmtLogService.generateReport(tplbasePath, docpathsegment, request.getHeader(), request.getBody().getData());
                if (filePath != null) {
    ...
    @POST
    @Path("/get/indicators")
    public Object getKeyPerformanceIndicators(ManageITRequest<GlobCostmgmtLogSearchDefParam> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null) {
                log.error("Wrong Request.");
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong Request.");
            }
            return globCostmgmtLogService.generateKeyPerformanceIndicators(request.getHeader(), request.getBody().getData());
        }
        return Response.status(Response.Status.BAD_REQUEST).build();
    }
}
