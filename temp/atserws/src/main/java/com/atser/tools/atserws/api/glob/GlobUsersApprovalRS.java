...
import com.atser.tools.atserejb.service.glob.GlobUsersApprovalService;
 ...
 * @author droldan
 */
@Path("/glob/usersapproval")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobUsersApprovalRS implements IBaseWSException {

    @EJB
    private GlobUsersApprovalService globUsersApprovalService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globUsersApprovalService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globUsersApprovalService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globUsersApprovalService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globUsersApprovalService.list(request.getBody());
        }
    ...
    public Object findById(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams().containsKey("id")) {
                return globUsersApprovalService.findById(request.getBody().getParams().get("id").toString());
            } else {
    ...
    @PermitAll
    @Path("/crud")
    public Object crud(ManageITRequest<GlobUsersApproval> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (StringSupport.isNotNullAndNotEmpty(request.getBody().getAction())) {
                ...
                switch (action) {
                    ...
                    case "remove": {
                        ...
                        globUsersApprovalService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        globUsersApprovalService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        globUsersApprovalService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    @POST
    @Path("/approve")
    public Object approveUser(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getParams() == null || !request.getBody().getParams().isContainsAllKeys(new String[]{"id", "status"})) {
                throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
            }
            globUsersApprovalService.approveUser(request.getHeader(), request.getBody(), env.getEmailTplEngine());
        }
    ...
    @GET
    @Path("/findby/username/{username}")
    public Object findApprovedUserByUsername(@PathParam("username") String username) throws IOException {
        if (StringSupport.isNullOrEmpty(username)) {
            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
        }
        Optional<GlobUsersApproval> user = globUsersApprovalService.findByUsername(username);
        if (!user.isPresent()) {
            throwBaseException(Response.Status.NOT_FOUND, "User not found");
        }
