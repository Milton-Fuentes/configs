...
import com.atser.tools.atserejb.service.glob.GlobContractorPersonnelService;
 ...
 * @author droldan
 */
@Path("/glob/contractorpersonnel")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class GlobContractorPersonnelRS implements IBaseWSException {

    @EJB
    GlobContractorPersonnelService globContractorPersonnelService;
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globContractorPersonnelService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return globContractorPersonnelService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return globContractorPersonnelService.list(request.getBody());
        }
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<GlobContractorPersonnel> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return globContractorPersonnelService.findById(request.getBody().getParams().get("id").toString());
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        globContractorPersonnelService.delete(request.getBody().getParams().get("id").toString());
                    ...
                    case "insert": {
                        ...
                        globContractorPersonnelService.save(request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        globContractorPersonnelService.update(request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object changeStatus(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            ...
            globContractorPersonnelService.changeEnabled(request.getBody());
        }
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return globContractorPersonnelService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return globContractorPersonnelService.getRevisions(id);
    }
    ...
    public Response generateContractorReport(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws IOException, Exception {
        ...
        if (StringSupport.isNotNullAndNotEmpty(paramReportId)) {
            ...
            String filePath = globContractorPersonnelService.generateReport(paramReportId, tplbasePath, docpathsegment, paramDocType, request.getHeader().getClientid(), request.getBody());
            if (filePath != null) {
