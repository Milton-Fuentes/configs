...
import com.atser.tools.atserejb.service.qa.QaaspmMixdesignsJmfSievesService;
 ...
 * @author droldan
 */
@Path("/qa/aspmmixdesignsjmfsieves")
@WsSecured
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
@ManageITSegmentRest
@Slf4j
public class QaaspmMixdesignsJmfSievesRS implements IBaseWSException {
    ...
    public Object datatableList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignsJmfSievesService.dataTableList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object datatableGridList(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignsJmfSievesService.dataGridList(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByGet(@Context UriInfo uriInfo, @CookieParam("JSESSIONID") Cookie cookie) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignsJmfSievesService.list(ApiCommon.buildFilter(uriInfo));
    }
    ...
    public Object filterListByPost(ManageITRequest<Object> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            return qaaspmMixdesignsJmfSievesService.list(request.getBody());
        }
    ...
    @POST
    @Path("/save/batch")
    public Object crudBatch(ManageITRequest<List<VwQaaspmMixJmfsv>> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
            if (request.getBody().getData() == null || request.getBody().getData().isEmpty()) {
                throwBaseException(Response.Status.BAD_REQUEST, "No data.");
            }
            qaaspmMixdesignsJmfSievesService.updateBatch(request.getHeader(), request.getBody().getData());
            //return contextService.save(request.getBody().getData(), ((AuthUser)securityContext.getUserPrincipal()).getAdmUser());
    ...
    @POST
    @Path("/crud")
    public Object crud(ManageITRequest<QaaspmMixdesignsJmfSieves> request) throws JsonParseException, JsonMappingException, IOException {
        if (request != null && request.validate()) {
                ...
                switch (action) {
                    case "get": {
                        ...
                        if (request.getBody().getParams().containsKey("id") && StringSupport.isNotNullAndNotEmpty(request.getBody().getParams().get("id"))) {
                            return qaaspmMixdesignsJmfSievesService.findById(request.getHeader(), request.getBody().getParams().get("id").toString());
                        } else {
                            throwBaseException(Response.Status.BAD_REQUEST, "Wrong parameters.");
                        }
                        break;
                    ...
                    case "remove": {
                        ...
                        qaaspmMixdesignsJmfSievesService.delete(request.getBody().getParams().get("id").toString());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "insert": {
                        ...
                        qaaspmMixdesignsJmfSievesService.save(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                    case "update": {
                        if (request.getBody().getData() == null) {
                        ...
                        qaaspmMixdesignsJmfSievesService.update(request.getHeader(), request.getBody().getData());
                        return Response.status(Response.Status.OK).build();
                    }
                }
            } else {
    ...
    public Object getVersion(@QueryParam("n") String number) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignsJmfSievesService.getVersion(Integer.valueOf(number));
    }

    @GET
    @Path("/version/list")
    public Object getVersionList(@QueryParam("id") String id) throws JsonParseException, JsonMappingException, IOException {
        return qaaspmMixdesignsJmfSievesService.getRevisions(id);
    }
}
