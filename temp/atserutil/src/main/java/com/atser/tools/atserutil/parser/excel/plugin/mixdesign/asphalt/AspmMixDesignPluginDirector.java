 ...
 */
package com.atser.tools.atserutil.parser.excel.plugin.mixdesign.asphalt;
...
import com.atser.tools.atserutil.parser.excel.plugin.mixdesign.asphalt.strategy.AspmMixDesignContext;
 ...
 * @author droldan
 */
public class AspmMixDesignPluginDirector implements Serializable {

    private ExcelContext pExcelCtx;
    private ExcelDocument pDoc;
    private eAspmMixDesign pTemplate;
    private AspmMixDesignContext context;

    public AspmMixDesignPluginDirector(File pfile, eAspmMixDesign ptmp) throws FormatDocumentNotAllowedException, NotFoundMimeTypeException, FileNotFoundException {
        this.pExcelCtx = new ExcelContext(pfile);
        this.pTemplate = ptmp;
        this.pDoc = pExcelCtx.getPExcelDoc();
        this.context = new AspmMixDesignContext(this.pTemplate);
    }

    public List<AspmMixDesignDef> getModel() throws TemplateDifferentException, ExcelParsingException, IOException, EmptyDocumentException {
        List<AspmMixDesignDef> listmodel = new ArrayList<>();
    ...
    public List<AspmMixDesignDef> getModelAllSheets() throws TemplateDifferentException, ExcelParsingException, IOException, EmptyDocumentException {
        List<AspmMixDesignDef> listmodel = new ArrayList<>();
        for (Sheet page : pDoc.getListPages()) {
            listmodel.addAll(context.getpStrategy(page).getResourceData());
        }
    ...
    public List<AspmMixDesignDef> getModelPerSheet(String sheetName) throws TemplateDifferentException, ExcelParsingException, IOException, EmptyDocumentException {
        List<AspmMixDesignDef> listmodel = new ArrayList<>();
        for (Sheet page : pDoc.getListPages()) {
    ...
    public String getModelToJson() throws TemplateDifferentException, ExcelParsingException, IOException, EmptyDocumentException {
        List<AspmMixDesignDef> listmodel = getModel();
    ...
    public eAspmMixDesign getpTemplate() {
        return pTemplate;
    }

    public void setpTemplate(eAspmMixDesign pTemplate) {
