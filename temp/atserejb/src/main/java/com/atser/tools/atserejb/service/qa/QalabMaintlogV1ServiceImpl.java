...
import com.atser.tools.atserejb.dao.qa.QalabMaintlogV1Dao;
...
@Stateless
public class QalabMaintlogV1ServiceImpl implements QalabMaintlogV1Service, IBaseEJBException {
    
    private final String rootFolderName = "Maintenance";
    
    @Inject
    private QalabMaintlogV1Dao dao;
    
    @Inject
    private QalabMainttypeV1Service qalabMainttypeService;
    ...
    @Override
    public QalabMaintlogV1 findById(String id) {
        return findById(null, NumberSupport.getBigDecimal(id));
    }
    
    @Override
    public QalabMaintlogV1 findById(BigDecimal id) {
        return findById(null, id);
    }
    
    @Override
    public QalabMaintlogV1 findById(ManageITHeader header, String id) {
        return findById(header, NumberSupport.getBigDecimal(id));
    }
    
    @Override
    public QalabMaintlogV1 findById(ManageITHeader header, BigDecimal id) {
        if (header == null) {
            return dao.findById(id);
        } else {
            QalabMaintlogV1 elem = dao.findById(id);
    ...
    @Override
    public BigDecimal save(ManageITHeader header, QalabMaintlogV1 elem) {
        if (elem != null) {
    ...
    private Boolean existMaintDate(BigDecimal eqId, BigDecimal eqMPartId, Date maintDate) {
        ...
        List<QalabMaintlogV1> filterList = list(search);
    ...
    private Date getMaintDueDate(QalabMaintlogV1 elem) {
        QalabMainttypeV1 mtype = qalabMainttypeService.findById(elem.getEquipMaintTypeid().getId().toString());
        Date dueDate = DateSupport.addChronoUnitToDate(elem.getMaintDate(), ChronoUnit.MONTHS, mtype.getIntervalM().longValue());
        return dueDate;
    }
    
    @Override
    public void update(QalabMaintlogV1 elem) {
        if (elem != null) {
            QalabMaintlogV1 model = findById(elem.getId());
    ...
    public void delete(String id) {
        if (id != null) {
            QalabMaintlogV1 elem = findById(id);
            if (elem != null) {
    ...
    @Override
    public List<QalabMaintlogV1> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataTableWrapper<QalabMaintlogV1> dataTableList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            Long filtered;
            List<QalabMaintlogV1> finalList = list(request);
    ...
    private BigDecimal createFolderStruct(String userId, String clientId, QalabMaintlogV1 elem) {
    ...
    @Override
    public void update(ManageITHeader header, QalabMaintlogV1 elem) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    
    @Override
    public DataGridWrapper<QalabMaintlogV1> dataGridList(ManageITBodyRequest request) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
}
