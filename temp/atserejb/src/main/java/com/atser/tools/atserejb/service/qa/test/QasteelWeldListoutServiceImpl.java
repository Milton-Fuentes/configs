...
import com.atser.tools.atserejb.dao.qa.test.QasteelWeldListoutDao;
...
@Stateless
public class QasteelWeldListoutServiceImpl implements QasteelWeldListoutService, IBaseEJBException {

    @Inject
    private QasteelWeldListoutDao dao;

    @Override
    public QasteelWeldListout findById(String id) {
        return dao.findById(NumberSupport.getBigDecimal(id));
    }

    @Override
    public QasteelWeldListout findById(BigDecimal id) {
        return dao.findById(id);
    }

    @Override
    @SaveMethod
    public BigDecimal save(ManageITHeader header, QasteelWeldListout elem) {
        if (elem != null) {
    ...
    @Override
    public void update(ManageITHeader header, QasteelWeldListout elem) {
        if (elem != null) {
            QasteelWeldListout model = findById(elem.getId());
    ...
    private QasteelWeldListout populate(QasteelWeldListout model, QasteelWeldListout elem) {
        //---commons -----------------------------------------------------------
        ...
        //---test Out Fields----------------------------------------------------
        model.setWelderId(elem.getWelderId());
        model.setVisualResult(elem.getVisualResult());
        model.setWeldedDate(elem.getWeldedDate());
        model.setJointType(elem.getJointType());
    ...
    @Override
    public List<QasteelWeldListout> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataGridWrapper<QasteelWeldListout> dataGridList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            List<QasteelWeldListout> finalList = list(request);
    ...
    @Override
    public DataTableWrapper<QasteelWeldListout> dataTableList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            Long filtered;
            List<QasteelWeldListout> finalList = list(request);
    ...
    @Override
    public void calcTest(QasteelWeldListout model) {
