...
import com.atser.tools.atserejb.dao.qa.QalabCaliblogV2Dao;
...
@Stateless
public class QalabCaliblogV2ServiceImpl implements QalabCaliblogV2Service, IBaseEJBException {

    private final String rootFolderName = "Calibration";

    @Inject
    private QalabCaliblogV2Dao dao;
    ...
    @Override
    public QalabCaliblogV2 findById(BigDecimal id) {
        return findById(null, id);
    }

    @Override
    public QalabCaliblogV2 findById(String id) {
        return findById(null, NumberSupport.getBigDecimal(id));
    }

    @Override
    public QalabCaliblogV2 findById(ManageITHeader header, String id) {
        return findById(header, NumberSupport.getBigDecimal(id));
    }

    @Override
    public QalabCaliblogV2 findById(ManageITHeader header, BigDecimal id) {
        if (header == null) {
            return dao.findById(id);
        } else {
            QalabCaliblogV2 elem = dao.findById(id);
    ...
    @Override
    public BigDecimal save(ManageITHeader header, QalabCaliblogV2 elem) {
        if (elem != null) {
    ...
    private void updateExpirationDate(BigDecimal eqId) {
        ...
        List<QalabCaliblogV2> filterList = list(searchLog);
        if (!filterList.isEmpty()) {
            Comparator<QalabCaliblogV2> sByCalibDate = (QalabCaliblogV2 o1, QalabCaliblogV2 o2) -> o1.getCalibDate().compareTo(o2.getCalibDate());
            filterList = filterList.stream().sorted(sByCalibDate.reversed()).collect(Collectors.toList());
            QalabCaliblogV2 modellog = filterList.get(0);
            QalabEquipinfoV2 modelInfo = qalabEqInfoService.findById(modellog.getLabEquipId().getId().toString());
            ...
            QalabEquipinfoV2 modelInfo = qalabEqInfoService.findById(eqId.toString());
    ...
    private Boolean existCalibDate(BigDecimal eqId, Date calibDate) {
        ...
        List<QalabCaliblogV2> filterList = list(search);
        if (!filterList.isEmpty()) {
    ...
    @Override
    public QalabCaliblogV2 getLastCalibrationLog(BigDecimal eqId) {
        ManageITBodyRequest searchLog = new ManageITBodyRequest(0, 0);
        searchLog.filter("labEquipId.id", eqId);
        List<QalabCaliblogV2> filterList = list(searchLog);
    ...
    @Override
    public void update(QalabCaliblogV2 elem) {
        if (elem != null) {
            QalabCaliblogV2 model = findById(elem.getId());
    ...
    public void delete(String id) {
        if (id != null) {
            QalabCaliblogV2 elem = findById(id);
            if (elem != null) {
    ...
    @Override
    public List<QalabCaliblogV2> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataTableWrapper<QalabCaliblogV2> dataTableList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            Long filtered;
            List<QalabCaliblogV2> finalList = list(request);
    ...
    private BigDecimal createFolderStruct(String userId, String clientId, QalabCaliblogV2 elem) {
    ...
    @Override
    public void update(ManageITHeader header, QalabCaliblogV2 elem) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public DataGridWrapper<QalabCaliblogV2> dataGridList(ManageITBodyRequest request) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
}
