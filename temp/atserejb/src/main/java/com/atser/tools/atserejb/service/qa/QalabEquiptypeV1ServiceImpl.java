...
import com.atser.tools.atserejb.dao.qa.QalabEquiptypeV1Dao;
...
@Stateless
public class QalabEquiptypeV1ServiceImpl implements QalabEquiptypeV1Service, IBaseEJBException {

    @Inject
    private QalabEquiptypeV1Dao dao;

    @Inject
    private GlobClientEquipcatgService globClientEquipcatgService;

    @Override
    public QalabEquiptypeV1 findById(String id) {
        return dao.findById(NumberSupport.getBigDecimal(id));
    }

    @Override
    public QalabEquiptypeV1 findById(BigDecimal id) {
        return dao.findById(id);
    }

    @Override
    public BigDecimal save(ManageITHeader header, QalabEquiptypeV1 elem) {
        save(elem);
        return BigDecimal.ZERO;
    }

    @Override
    public void update(ManageITHeader header, QalabEquiptypeV1 elem) {
        update(elem);
    }

    @Override
    public void save(QalabEquiptypeV1 elem) {
        if (elem != null) {
    ...
    @Override
    public void update(QalabEquiptypeV1 elem) {
        if (elem != null) {
            QalabEquiptypeV1 model = findById(elem.getId());
    ...
    @Override
    public List<QalabEquiptypeV1> list(ManageITHeader header, ManageITBodyRequest body) {
        List<QalabEquiptypeV1> list;
        ...
        if (header != null && StringSupport.isNotNullAndNotEmpty(header.getClientid())) {
            ...
            if (catgList != null && catgList.size() > 0) {
                list.forEach(it -> {
                    Optional<GlobClientEquipcatg> match = catgList.stream().filter(p -> p.getEquipCatgId().getId().equals(it.getCatgId().getId())).findFirst();
    ...
    @Override
    public List<QalabEquiptypeV1> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataTableWrapper<QalabEquiptypeV1> dataTableList(ManageITBodyRequest request) {
        return dataTableList(null, request);
    }

    @Override
    public DataTableWrapper<QalabEquiptypeV1> dataTableList(ManageITHeader header, ManageITBodyRequest body) {
        if (body != null && (body.getFilter() != null || body.getSorted() != null)) {
            Long count = dao.getTotal(body.getFilterWithOutLikes());
            Long filtered;
            List<QalabEquiptypeV1> finalList = list(header, body);
    ...
    public void changeStatus(ManageITBodyRequest request) {
        ...
        QalabEquiptypeV1 model = findById(request.getParams().get("id").toString());
    ...
    @Override
    public DataGridWrapper<QalabEquiptypeV1> dataGridList(ManageITBodyRequest request) {
