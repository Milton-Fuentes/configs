...
import com.atser.tools.atserejb.dao.qa.QalabEquipinfoProjectV1Dao;

@Stateless
public class QalabEquipinfoProjectV1ServiceImpl implements QalabEquipinfoProjectV1Service, IBaseEJBException {

    @Inject
    private QalabEquipinfoProjectV1Dao dao;

    @Inject
    private QalabEquipinfoV1Service qalabEquipinfoService;
    ...
    @Override
    public QalabEquipinfoProjectV1 findById(String id) {
        return dao.findById(NumberSupport.getBigDecimal(id));
    }

    @Override
    public QalabEquipinfoProjectV1 findById(BigDecimal id) {
        return dao.findById(id);
    }

    @Override
    public BigInteger save(ManageITHeader header, QalabEquipinfoProjectV1 elem) {
        if (elem != null) {
            if (!dao.exists(elem.getEquipId().getId().toBigInteger(), elem.getProjectId())) {
                QalabEquipinfoV1 equipId = qalabEquipinfoService.findById(StringSupport.getString(elem.getEquipId().getId()));
    ...
    @Override
    public void update(ManageITHeader header, QalabEquipinfoProjectV1 elem) {
        if (elem != null) {
            QalabEquipinfoProjectV1 model = findById(elem.getId());
    ...
    private QalabEquipinfoProjectV1 populate(QalabEquipinfoProjectV1 model, QalabEquipinfoProjectV1 elem) {
    ...
    @Override
    public List<QalabEquipinfoProjectV1> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataGridWrapper<QalabEquipinfoProjectV1> dataGridList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            List<QalabEquipinfoProjectV1> finalList = list(request);
    ...
    @Override
    public DataTableWrapper<QalabEquipinfoProjectV1> dataTableList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            Long filtered;
            List<QalabEquipinfoProjectV1> finalList = list(request);
    ...
    private String generateEquipLogReport(String tplbasePath, String tplpath, String docpathsegment, String docType, ManageITBodyRequest request, Map<String, Object> paramsJasper) {
        ...
        try {
            ...
            Logger.getLogger(QalabEquipinfoProjectV1ServiceImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }

    private List<EquipLogTO> generateEquipLogReportData(ManageITBodyRequest request) {
        List<QalabEquipinfoProjectV1> filterList = list(request);
