...
import com.atser.tools.atserejb.dao.qa.QalabEquipinfoV1Dao;
...
import com.atser.tools.atserejb.model.QalabEquipclienttestV1;
...
@Stateless
@Slf4j
public class QalabEquipinfoV1ServiceImpl implements QalabEquipinfoV1Service, IBaseEJBException {

    private final String rootFolderName = "Equipments";

    @Inject
    private QalabEquipinfoV1Dao dao;

    @Inject
    private VwClientTestsService vwClientTestsService;

    @Inject
    private QalabCaliblogV1Service qalabCaliblogService;

    @Inject
    private GlobFolderService folderService;

    @Inject
    private QalabEquiptypeV1Service equipTypeService;
    ...
    @Inject
    private QalabEquipinfoProjectV1Service qalabEquipinfoProjectService;

    @Resource(mappedName = "java:jboss/mail/Atser")
    private Session mailSession;

    @Override
    public QalabEquipinfoV1 findById(String id) {
        return dao.findById(NumberSupport.getBigDecimal(id));
    }

    public QalabEquipinfoV1 findById(BigDecimal id) {
        return dao.findById(id);
    }

    @Override
    public void save(ManageITHeader header, QalabEquipinfoV1 elem) {
        if (elem != null) {
            elem.setLabId(globLabsService.findById(elem.getLabId().getId()));
    ...
    @Override
    public void update(QalabEquipinfoV1 elem) {
        update(null, elem);
    }

    @Override
    public void update(ManageITHeader header, QalabEquipinfoV1 elem) {
        if (elem != null) {
            QalabEquipinfoV1 model = findById(elem.getId());
            ...
            model.setLabId(globLabsService.findById(elem.getLabId().getId()));
            ...
            List<QalabEquipclienttestV1> currentList = new ArrayList<>();
    ...
    @Override
    public void updateExpirationDate(QalabEquipinfoV1 elem) {
        if (elem != null) {
            QalabEquipinfoV1 model = findById(elem.getId());
    ...
    @Override
    public List<QalabEquipinfoV1> list(ManageITBodyRequest request) {
    ...
    @Override
    public DataTableWrapper<QalabEquipinfoV1> dataTableList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            Long filtered;
            List<QalabEquipinfoV1> finalList = list(request);
    ...
    @Override
    public DataGridWrapper<QalabEquipinfoV1> dataGridList(ManageITBodyRequest request) {
        if (request != null && (request.getFilter() != null || request.getSorted() != null)) {
            Long count = dao.getTotal(request.getFilterWithOutLikes());
            List<QalabEquipinfoV1> finalList = list(request);
    ...
    public List<MaintLogPojo> listMaintDueDate(String id) {
        ...
        QalabEquipinfoV1 einfo = findById(id);
        Comparator<QalabMaintlogV1> sByMaintDate = (QalabMaintlogV1 o1, QalabMaintlogV1 o2) -> o1.getMaintDate().compareTo(o2.getMaintDate());
        if (einfo != null) {
            List<QalabMainttypeV1> mainttypeList = einfo.getEquipId().getQalabMainttypeList();
            List<QalabMaintlogV1> maintlogList = einfo.getQalabMaintlogList();
            if (mainttypeList != null && !mainttypeList.isEmpty()) {
                if (maintlogList != null && !maintlogList.isEmpty()) {
                    mainttypeList.forEach(it -> {
                        List<QalabMaintlogV1> finalList = maintlogList.stream().filter(p -> p.getEquipMaintTypeid().equals(it)).collect(Collectors.toList());
    ...
    public void changeStatus(ManageITBodyRequest request) {
        ...
        QalabEquipinfoV1 model = findById(request.getParams().get("id").toString());
    ...
    private EquipLogPojo convertToEquipLogPojo(QalabEquipinfoV1 info) {
    ...
    private CalibLogPojo convertToCalibLogPojo(QalabEquipinfoV1 info) {
        ...
        QalabCaliblogV1 lastCalibLog = qalabCaliblogService.getLastCalibrationLog(info.getId());
    ...
    private List<EquipLogTO> generateEquipLogReportData(ManageITBodyRequest request) {
        List<QalabEquipinfoV1> filterList = list(request);
    ...
    private EquipLogTO convertToEquipLogTO(QalabEquipinfoV1 info) {
    ...
    private List<MaintLogWithEquipLogTO> convertToMaintLogWithEquipLogTO(QalabEquipinfoV1 info) {
        List<MaintLogWithEquipLogTO> rList = new ArrayList<>();
    ...
    @Override
    public BigDecimal createEquipFolderStruct(String userId, String clientId, QalabEquipinfoV1 elem) {
        if (StringSupport.isNullOrEmpty(clientId)) {
            throwBaseException(Response.Status.BAD_REQUEST, "Wrong Parameters.");
        }
        QalabEquiptypeV1 equipType;
    ...
    private void updateEquipFolderName(String clientId, QalabEquipinfoV1 dbItem, QalabEquipinfoV1 updateItem) {
        if (StringSupport.isNullOrEmpty(clientId)) {
            return;
        }
        QalabEquiptypeV1 equipType;
    ...
    public void assignToProject(ManageITHeader header, String projectId, String equipId) {
        ...
        QalabEquipinfoV1 elem = findById(equipId);
        if (elem != null) {
            if (!qalabEquipinfoProjectService.exists(NumberSupport.getBigInteger(projectId), elem.getId().toBigInteger())) {
                qalabEquipinfoProjectService.save(header, new QalabEquipinfoProjectV1(elem, NumberSupport.getBigInteger(projectId)));
            } else {
    ...
    private String generateEquipBarcodePrintLog(String tplbasePath, String tplpath, String docpathsegment, String docType, BigInteger reqId, Map<String, Object> paramsJasper) {
        ...
        QalabEquipinfoV1 model = findById(StringSupport.getString(reqId));
        if (model == null) {
            return null;
        }
